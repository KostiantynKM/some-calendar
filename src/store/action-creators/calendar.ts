    ...
    return async (dispatch: Dispatch<CalendarAction>) => {
        try {
            ...
            if(currentCountry){
                ...
                dispatch({type: CalendarActionTypes.SETUP_CALENDAR, payload: {days,month:moment().format("MMM-YYYY"),firstDay:moment().startOf('month').day(),error:null}});
            }else {
                for (let i = 1; i <= moment().daysInMonth(); i++) {
...
export const changeTaskIn = (payload: { start: number; draggableId: number; end: number; day: string; sourceDest?:string }) => {
    return async (dispatch: Dispatch<CalendarAction>,getState: () => GlobalState) => {
        const state = getState();
